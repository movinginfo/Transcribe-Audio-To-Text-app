    ...
    const char* cache_dir)
{
    ...
    try {
        ...
        // Build per-device compile config.
        // LATENCY mode + num_requests=1 is critical for NPU and beneficial for GPU:
        // without it the NPU defaults to THROUGHPUT (pipeline) mode which adds
        // significant latency overhead for single-inference workloads.
        ov::AnyMap config;
        std::string dev_str(device);
        // Strip compound prefixes (MULTI:NPU,GPU  HETERO:NPU,CPU  AUTO:NPU,GPU)
        // to extract the primary device name for the hint decision.
        std::string primary = dev_str;
        for (const auto& prefix : {"MULTI:", "HETERO:", "AUTO:"}) {
            if (dev_str.substr(0, std::string(prefix).size()) == prefix) {
                primary = dev_str.substr(std::string(prefix).size());
                auto comma = primary.find(',');
                if (comma != std::string::npos) primary = primary.substr(0, comma);
                break;
            }
        }
        if (primary == "NPU" || primary == "GPU" || primary == "CPU" || primary == "AUTO") {
            config[ov::hint::performance_mode.name()] = ov::hint::PerformanceMode::LATENCY;
            config[ov::hint::num_requests.name()]     = 1u;
        }

        // Produce a compiled-model object, given the device ("CPU", "GPU", etc.)
        auto compiledModel = core.compile_model(model, device, config);

        // Print the actual execution device(s) OpenVINO selected.
        // For AUTO/MULTI this may differ from the requested device string.
        // For explicit NPU/GPU/CPU it confirms the device is truly in use.
        try {
            auto exec_devs = compiledModel.get_property(ov::execution_devices);
            fprintf(stderr, "%s: actual execution device(s):", __func__);
